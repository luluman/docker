...
if [ "$color_prompt" = yes ]; then
    PS1='$(__exit_status_glyph color) ${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]:\[\033[01;34m\]\w\[\033[00m\]\$ '
else
    PS1='$(__exit_status_glyph) ${debian_chroot:+($debian_chroot)}\u@\h:\w\$ '
fi
...
if [[ ! -d ~/.terminfo ]] && [[ -f /usr/local/share/bash-color/terminfo-24bit.src ]]; then
...
[ -f "$HOME/.cargo/env" ] && . "$HOME/.cargo/env" # rust-env
